...
17. ✅ Partner invite spine (2026-04-16, Tasks 1–24 across three sessions)
    - **Routes:** `/invite` (Organizer compose/waiting toggler), `/invite/[token]` (public branded landing — 3 states: valid / invalid-or-cancelled / already-accepted), `/invite/[token]/accept` (authenticated redemption → `redirect("/today?welcomed=1")`).
    - **API:** `POST /api/invites` (create, Zod-validated email, one-active-per-household 409, optional Resend send), `GET /api/invites` (current active invite or null, polled every 5s), `DELETE /api/invites/[token]` (cancel — directory name says `[token]` to satisfy Next.js's sibling-dynamic-slug constraint with `[token]/accept/`, but the caller actually passes the invite UUID; see the route handler's comment), `POST /api/invites/[token]/accept` (redeem, maps 5-kind discriminated union to HTTP status), `POST /api/dev/reset-invite-state` (dev-only, gated on `NODE_ENV !== "production"` + `x-dev-reset-secret`).
    ...
    - **Email:** `renderInviteEmail({ organizerName, inviteUrl })` returns `{ subject, text, html }` with HTML-escaped organizer name. `sendEmail` is a thin Resend wrapper with a **dev-mode fallback** — when `RESEND_API_KEY` or `EMAIL_FROM` is missing it logs `[email:dev-fallback] would send:` instead of throwing, which is what makes the e2e flow work without a live API key.
    ...
    - **Link chrome:** banner CTA, mobile-header UserPlus icon, and sidebar collapsed-peek icon all route via `next/link` `<Link>` so the shared `(views)` layout survives navigation. Sidebar's **expanded-state** `motion.a` remains a raw anchor pending a motion-wrapped-Link pass — flagged in open-questions.
    - **Testing:** Playwright two-context fixture — `tests/invite.setup.ts` signs a second Clerk test user in and saves `tests/.auth/partner.json`; `tests/invite-flow.spec.ts` launches organizer + partner browser contexts and exercises send→land→accept→merged-household and the cancel-and-start-over path. `test.describe.configure({ mode: "serial" })` prevents races on the shared organizer household given global `fullyParallel: true`. The spec wraps browsers in `try/finally` so `browser.close()` runs on assertion failure.
...
18. ✅ Invite UX follow-ups (2026-04-17, Tasks 1–25)
    - **Notifications UI:** `NotificationBell` mounts in mobile header + sidebar utility row when paired; popover (desktop) / bottom-sheet (mobile) with `NotificationList`. Bell shows unread badge capped "9+"; opening marks-all-read via optimistic TanStack mutation. New routes: `GET /api/notifications`, `POST /api/notifications/read-all`, `PATCH /api/notifications/[id]/read`, `POST /api/invites/[token]/resend` (resend wraps the existing `sendEmail` + `renderInviteEmail`; path uses `[token]` to satisfy Next.js sibling-dynamic-slug constraint but the caller passes the invite UUID).
    - **Server-side generation:** `dispatchTaskAssigned` + `dispatchTaskCompletedByPartner` wired into `POST /api/tasks`, `PATCH /api/tasks/[id]`, `POST /api/tasks/[id]/complete`. Solo-guarded via new `householdIsPaired()` helper. Skips self-assign, sentinel-assignee, self-completion. `partner_joined` was already wired in `acceptInvite()` from Phase 17.
    - **Recognition:** `TaskListItem` gained optional `completedByLabel` prop; `DoneAccordion` resolves via `toUITask(task, me, partner, ...)` which returns `completedByLabel` only when a partner exists. Solo mode suppresses.
    - **First-run reveal:** `RevealBanner` on `/today?welcomed=1` — adaptive copy (pre-assigned count > 0 vs = 0), one-shot `localStorage.reveal-dismissed` flag, `router.replace("/today")` cleans the query param, CTA scrolls to + highlights the first assigned task.
    - **Solo-state polish:** `EmptyState` gained `theirs-solo` variant with 4 rotating copy variants and a "Bring your person in →" link to `/invite`. `ReengageBanner` fires after 7 days on a pending invite, once per invite ID via `localStorage.reengage-dismissed-<inviteId>`; cancelling + re-creating an invite resets the flag for the new ask.
    - **Helpers:** `scrollToTaskAndHighlight(taskId)` via `data-task-id` attribute; shared by RevealBanner CTA + notification row clicks. Keyframe `task-highlight-flash` in `globals.css`.
    - **Lint discipline:** `react-hooks/set-state-in-effect` rule forced two notable patterns — `NotificationBell` uses `useSyncExternalStore` for its matchMedia isMobile detection, and `RevealBanner`/`ReengageBanner` use the existing `useIsoLayoutEffect` pattern pioneered by `InviteBanner` to read localStorage pre-paint.
    - **Tests:** Extended `tests/invite-flow.spec.ts` with 4 new two-context cases (reveal banner, notification on assign, notification on partner-complete, Done-accordion attribution). New `tests/solo-state.spec.ts` with 2 cases (Theirs-solo empty state, re-engage banner via clock-shift to 8 days). The new `invite-flow.spec.ts` cases inherit the pre-existing `E2E_PARTNER_*` env-var dependency flagged in Phase 17.
19. Keep updating docs after every major change
